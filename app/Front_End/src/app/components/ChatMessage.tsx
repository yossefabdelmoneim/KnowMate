  ...
  const renderContent = (content: string) => {
    ...
    return lines.map((line, i) => {
      ...
      if (line.startsWith("> ")) return (
        <blockquote key={i} className="border-l-2 border-primary/50 pl-3 my-2 text-muted-foreground italic text-sm">
          {line.slice(2)}
        </blockquote>
      );
      if (line.startsWith("- ")) return (
        <li key={i} className="ml-4 text-sm text-foreground/90 my-0.5 list-disc">{line.slice(2).replace(/\*\*(.*?)\*\*/g, "$1")}</li>
      );
      if (/^\d+\./.test(line)) return (
        <li key={i} className="ml-4 text-sm text-foreground/90 my-0.5 list-decimal">{line.replace(/^\d+\.\s*/, "").replace(/\*\*(.*?)\*\*/g, "$1")}</li>
      );
      if (line.startsWith("*Source:") || line.startsWith("*")) return (
        <p key={i} className="text-xs text-muted-foreground mt-3 italic">{line.replace(/\*/g, "")}</p>
      );
      if (line.trim() === "") return <br key={i} />;

      const boldProcessed = line.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>");
      return <p key={i} className="text-sm text-foreground/90 my-1" dangerouslySetInnerHTML={{ __html: boldProcessed }} />;
    });
  };
        ...
        <div className="max-w-[75%]">
          {msg.files && msg.files.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-2 justify-end">
              {msg.files.map((f, i) => (
                <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-primary/10 text-primary text-xs rounded-lg font-medium">
                  <FileText size={11} /> {f}
                </span>
              ))}
            </div>
            ...
            {msg.citations && msg.citations.length > 0 && (
              <div className="mt-4 pt-3 border-t border-border">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {msg.citations.map((c) => (
                    <span key={c.id} className="text-xs px-2 py-0.5 bg-muted rounded-md text-muted-foreground">
                      [{c.id}] {c.source}
                    </span>
                  ))}
                </div>
